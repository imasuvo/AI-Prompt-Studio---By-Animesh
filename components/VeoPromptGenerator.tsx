...
import { generateTextPrompt, generateStoryboardPrompts, generateImage } from '../services/geminiService';
import { VEO_SYSTEM_PROMPT } from '../constants';
import { Spinner } from './Spinner';
import { PromptCard } from './PromptCard';
import { FilmIcon, HistoryIcon, ExpandIcon } from './Icons';
import { HistoryItem, HistoryItemType, StoryboardHistoryItem, StoryboardImage } from '../types';
...
export const VeoPromptGenerator: React.FC<VeoProps> = ({ history, setHistory, openLightbox }) => {
  const [idea, setIdea] = useState<string>('');
  const [currentItem, setCurrentItem] = useState<StoryboardHistoryItem | null>(null);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [loadingStep, setLoadingStep] = useState<string>('');
  const [error, setError] = useState<string>('');

  const localHistory = useMemo(() => 
    history.filter(item => item.type === HistoryItemType.STORYBOARD) as StoryboardHistoryItem[]
  , [history]);
  ...
  const handleHistoryClick = useCallback((item: StoryboardHistoryItem) => {
    setCurrentItem(item);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      setLoadingStep('Generating cinematic script...');
      const masterPrompt = await generateTextPrompt(VEO_SYSTEM_PROMPT, idea);

      setLoadingStep('Breaking script into keyframes...');
      const keyframePrompts = await generateStoryboardPrompts(masterPrompt);
      if (!keyframePrompts || keyframePrompts.length === 0) throw new Error('Could not generate keyframes.');
      ...
      for (let i = 0; i < keyframePrompts.length; i++) {
        ...
        const url = await generateImage(enrichedPrompt, '16:9');
  ...
  const renderStoryboard = (item: StoryboardHistoryItem) => (
    <div className="mt-8 animate-fade-in">
        <h3 className="font-semibold text-lg text-white mb-4">Generated Storyboard</h3>
        <div className="film-strip-container flex overflow-x-auto space-x-4 p-2 bg-brand-dark rounded-lg border border-brand-mid-gray">
            {item.storyboardImages.map((image, index) => (
                <div key={index} className="flex-shrink-0 w-64 sm:w-80 aspect-video relative group bg-black rounded-md overflow-hidden">
  ...
  return (
    <div className="bg-brand-gray p-4 sm:p-6 md:p-8 rounded-xl shadow-2xl border border-brand-mid-gray">
      <div className="flex items-center space-x-3 mb-4">
        <FilmIcon />
        <h2 className="text-xl sm:text-2xl font-bold text-white">VEO Storyboard Generator</h2>
      </div>
      <p className="text-gray-400 mb-6 text-sm sm:text-base">Enter a simple idea, and we'll generate a cinematic script and a visual storyboard.</p>
      
      <div className="space-y-4">
        <textarea
          value={idea}
          onChange={(e) => setIdea(e.target.value)}
          placeholder="e.g., a robot surfing on a wave of lava"
      ...
      {!isLoading && currentItem && (
        <div className="mt-6 space-y-6">
          <PromptCard prompt={currentItem.cinematicPrompt} title={`Cinematic VEO Script for "${currentItem.idea}"`} />
          {renderStoryboard(currentItem)}
        </div>
      )}
      ...
      {localHistory.length > 0 && (
         <div className="pt-8 mt-8 border-t border-brand-mid-gray">
            <div className="flex items-center space-x-3 mb-4">
                <HistoryIcon />
                <h3 className="text-xl font-bold text-white">Storyboard History</h3>
            </div>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-3 sm:gap-4">
              {localHistory.map((item) => (
                <div 
                  key={item.id} 
                  className={`group relative aspect-video rounded-lg overflow-hidden cursor-pointer border-2 hover:border-brand-purple transition-all duration-300 transform hover:scale-105 ${currentItem?.id === item.id ? 'border-brand-purple' : 'border-brand-mid-gray'}`}
                  onClick={() => handleHistoryClick(item)}
                  title={`Click to view. Idea: ${item.idea}`}
                >
                  <img src={item.storyboardImages[0]?.url} alt={item.idea} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-70 flex items-center justify-center transition-opacity duration-300">
                    <button onClick={(e) => { e.stopPropagation(); openLightbox(item.storyboardImages[0]); }} className="text-white p-2 rounded-full bg-black/50 hover:bg-black/75 transition-colors opacity-0 group-hover:opacity-100" title="View Image">
                        <ExpandIcon />
                    </button>
